...
controllers.controller('baseController', ['$scope', '$state','$filter', '$ionicHistory', '$ionicPlatform', '$rootScope', '$timeout', '$location', 'profileService', 'authService', 'connectedLearning.constants.environments', 'environmentData', 'connectedLearning.methods', 'connectedLearning.constants', '$ionicScrollDelegate', 'crittercismService', '$ionicSlideBoxDelegate', 'localStorageService',
                        // Merge from LearningEvents Injection, may not work since files lack - Booker(0912)
                        'menuService', '$ionicLoading','$sce', '$ionicModal', '$ionicPopup', '$cordovaClipboard', '$cordovaToast', 'streamService','impersonateService','itemPathService',
    function ($scope, $state, filter, $ionicHistory, $ionicPlatform, $rootScope, $timeout, $location, profileService, authService, environments, environmentData, methods, constants, $ionicScrollDelegate, crittercismService, $ionicSlideBoxDelegate, localStorageService,
              // Merge from LearningEvents Injection, may not work since files lack - Booker(0912)
              menuService, $ionicLoading, $sce, $ionicModal, $ionicPopup, $cordovaClipboard, $cordovaToast, streamService, impersonateService,itemPathService) {
        /// <summary>
        ...
        $scope.navigateProfile = function (peopleKey, enterpriseId, tabId) {
            ...
            $scope.navigateToState('app.people', { peopleKey: peopleKey, enterpriseId: enterpriseId, tab: tabId, title: 'empty', date: 'empty', location: 'empty' }, false);
        };
        ...
        $scope.navigateTraining = function (item, location, fromMyTraining) {
            ...
            $rootScope.activityID = $scope.currentTrainingId;
            ...
            if (fromMyTraining){
                $scope.navigateToState('app.trainingDetailTabs.activityTab', {selectedTraining: item, fromMyTraining: fromMyTraining}, true);
            }else{
