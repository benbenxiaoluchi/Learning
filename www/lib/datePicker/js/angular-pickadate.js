;(function(angular){
  ...
  angular.module('pickadate', ['pickadate.utils'])
    ...
    .directive('pickadate', ['$rootScope', '$locale', 'pickadateUtils', 'dateFilter', function ($rootScope, $locale, dateUtils, dateFilter) {
      return {
        require: 'ngModel',
        scope: {
          date: '=ngModel',
          ...
          '<div class="pickadate">' +
            '<div class="pickadate-header">' +
              '<div class="pickadate-controls">' +
                '<a href="" class="pickadate-prev" ng-click="changeMonth(-1)" ng-show="allowPrevMonth">< {{prevMonth | date:"MMMM"}}</a>' +
                '<a href="" class="pickadate-next" ng-click="changeMonth(1)" ng-show="allowNextMonth">{{nextMonth | date:"MMMM"}} ></a>' +
              '</div>'+
        ...
        link: function(scope, element, attrs, ngModel)  {
          ...
          scope.nextMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1);
          scope.prevMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() -1, 1);
          ...
          scope.render = function(initialDate) {
            ...
            for (var i = 0; i < allDates.length; i++) {
              ...
              // render range date
              if(((scope.toDate !== null) && ((date >= scope.fromDate) && (date <= scope.toDate))) || (date == scope.fromDate)){
                className = 'pickadate-date-inside-range';
              }
              if(((scope.isFrom) && (date > scope.toDate)) || ((scope.isTo) && (date < scope.fromDate))){
                className = ' pickadate-disabled';
              }
              // end render range date
              if (date === today) {
          ...
          scope.$watch(function() { return scope.isFrom },
            function() {scope.render(currentDate);}
          );
          scope.$watch(function() { return scope.isTo },
            function() {scope.render(currentDate);}
          );
          ...
          scope.setDate = function(dateObj) {
            ...
            setTimeout(function () {
                $rootScope.$broadcast('settedDate', dateObj.date);
            }, 300);
            
            if(scope.isFrom){scope.fromDate = dateObj.date;}
            if(scope.isTo){scope.toDate = dateObj.date;}
